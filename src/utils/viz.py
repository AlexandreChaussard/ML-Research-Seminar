...
def display_restoration_process(
        ...
        restored_data_list,
        max_samples=5,
        permutation_shape=(0, 1, 2)
):
    n_cols = max_samples
    fig, axs = plt.subplots(nrows=3, ncols=n_cols)
    fig.suptitle("Target, Altered, Restored data")
    for j in range(n_cols):
        axs[0][j].imshow(pytorch_to_numpy(target_data_list[0][j].permute(*permutation_shape)).squeeze())
        axs[1][j].imshow(pytorch_to_numpy(altered_data_list[0][j].permute(*permutation_shape)).squeeze())
        axs[2][j].imshow(pytorch_to_numpy(restored_data_list[0][j].permute(*permutation_shape)).squeeze())
