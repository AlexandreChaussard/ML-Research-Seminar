...
class WeightNormConv2d(nn.Module):
    def __init__(self, in_dim, out_dim, kernel_size, stride=1, padding=0,
        ...
        """Initializes a Conv2d augmented with weight normalization.
...
class RealNVP(nn.Module):
    def __init__(self, dataset_name, input_size, n_channels, prior, hps):
...
def train_realnvp(flow, optimizer, data_loader, n_epoch):
    ...
    for epoch in range(n_epoch):
        ...
        for batch_idx, data in enumerate(data_loader):
            ...
            x, log_det = utils.logit_transform(x)
