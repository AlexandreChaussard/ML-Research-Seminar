...
from src.utils.utils import split_feature, uniform_binning_correction
...
def train_glow(model, optimizer, data_train_loader, n_epoch):
    for epoch in range(n_epoch):
        ...
        for batch_idx, (data, _) in enumerate(data_train_loader):
            ...
            z, nll, y_logits = model(data, None)
            loss = torch.mean(nll)
            loss.backward()
...
def generate_data(model, temperature):
    return model(temperature=temperature, reverse=True)