...
class Flow_Encoder(torch.nn.Module):
    def __init__(self, channels_sizes,input_dim, z_dim, n_channels, strides):
        ...
        self.conv_net = []
        for i in range(len(channels_sizes) - 1):
            self.conv_net.append(nn.Conv2d(hidden_sizes[i], hidden_sizes[i + 1], stride=strides[i]))
            self.conv_net.append(nn.MaxPool2d((2,2)))
        ...
        num_features_before_fcnn = functools.reduce(
            operator.mul, 
            list(self.conv_net(torch.rand(1, *input_dim)).shape)
        )

        # self.linear_encoder = nn.Sequential(
        #     nn.Linear(in_features=num_features_before_fcnn, out_features=50),
        #     nn.ReLU(),
        #     nn.Linear(in_features=50, out_features=z_dim)
        # )

        self.mu_net = nn.Linear(num_features_before_fcnn, z_dim)
        self.sigma_net = nn.Linear(num_features_before_fcnn, z_dim)
